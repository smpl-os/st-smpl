...
hb_font_t *hbfindfont(struct wld_font *match);
...
hb_font_t *
hbfindfont(struct wld_font *match)
{
	for (int i = 0; i < hbfontcache.capacity; i++) {
	...
	FT_Face face = ((struct font *)match)->face;
...
void
hbtransform(HbTransformData *data, struct wld_font *xfont, const Glyph *glyphs, int start, int length)
{
	uint32_t mode;
